...
class CodeValidator {
  ...
  _extractFunctions(content, lines) {
    ...
    const patterns = [
      /^(?:export\s+)?(?:async\s+)?function\s+(\w+)\s*\(/m,  // function declarations
      /(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?(?:\([^)]*\)|[^=\s]+)\s*=>/m,  // arrow functions
      /(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?function/m,  // function expressions
      /(\w+)\s*:\s*(?:async\s+)?function/m,  // object methods
      /(?:async\s+)?(\w+)\s*\([^)]*\)\s*\{/m  // method definitions
    ];
